...
#     decay_steps = warmup_epoch,
#     end_learning_rate = FLAGS.lr,
#     power = 1.
# )
# lr_schedule = LRSchedule(0.001, warmup_epoch,
#                          steps_per_epoch = 56,
#                          decay_fn = lr_scheduler,
#                          continue_epoch = 0)

# optim = tf.keras.optimizers.Adadelta(0.0001)
...
@tf.function
def cal_loss(model, images, labels, objectiness, class_im_plain, gamma_factor, gamma_factor2, ob_imbal_labels_buf, ignore_label):
    ...
    with tf.GradientTape() as tape2:
        ...
        weed_alpha = (class_im_plain[1] + ob_imbal_labels_buf[0]) / 2.
        
        # need to add one loss
        ...
        # seg_loss2 = tf.nn.sigmoid(crop_predict[:, 1])* tf.math.log(tf.nn.sigmoid(crop_predict[:, 0]) + 1e-7) \
        #     + (1 - tf.nn.sigmoid(weed_predict[:, 1])) * tf.math.log(1 - tf.nn.sigmoid(weed_predict[:, 0]) + 1e-7)
            
        loss = ((no_obj_loss + obj_loss)) + (seg_loss)
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                ...
                gamma_factor2 = np.zeros([2], np.float32)
                gamma_factor2[0] = round(ob_imbal_labels_buf[0], 2)+ 1.
                gamma_factor2[1] = round(ob_imbal_labels_buf[1], 2)+ 1.
                ...
                # _ = cal_loss2(model, batch_images, batch_labels, objectiness, class_im_plain, 2)
                loss = cal_loss(model, batch_images, batch_labels, objectiness, temp_imbal, gamma_factor, gamma_factor2, temp_ob_imbal, 2)
                if count % 10 == 0:
