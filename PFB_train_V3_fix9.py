...
def cal_loss(model, images, labels, objectiness, class_im_plain, ob_imbal_labels_buf, ignore_label):
    ...
    with tf.GradientTape() as tape2:
        ...
        no_obj_loss = -(1. - no_obj_labels) * tf.math.log(1 - tf.nn.sigmoid(no_logit_objectiness) + 1e-7) * ob_imbal_labels_buf[1] + dice_loss(1 - no_obj_labels, 1 - tf.nn.sigmoid(no_logit_objectiness))
        ...
        obj_loss = -yes_obj_labels * tf.math.log(tf.nn.sigmoid(yes_logit_objectiness) + 1e-7) * ob_imbal_labels_buf[0] + dice_loss(yes_obj_labels, tf.nn.sigmoid(yes_logit_objectiness))
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                ...
                loss = cal_loss(model, batch_images, batch_labels, objectiness, temp_imbal, ob_imbal_labels_buf, 2)
                if count % 10 == 0:
