...
def cal_loss(model, images, labels, crop_objectiness, weed_objectiness, total_objectiness, class_im_plain, crop_im_plain, weed_im_plain, ignore_label):
    ...
    with tf.GradientTape() as tape:
        ...
        raw_logits = tf.reshape(logits, [-1, 4])
        ...
        crop_loss = dice_loss(cast_crop_objectiness, crop_logit) \
            + tf.nn.sigmoid_cross_entropy_with_logits(cast_crop_objectiness, crop_logit)
        crop_loss = tf.reduce_mean(crop_loss)

        weed_logit = raw_logits[:, 3]
        ...
        weed_loss = dice_loss(cast_weed_objectiness, weed_logit) \
            + tf.nn.sigmoid_cross_entropy_with_logits(cast_weed_objectiness, weed_logit)
...
def main():
    ...
    out = tf.keras.layers.Conv2D(4, (1,1), name="output_layer")(out)
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                ...
                if count % 100 == 0:
                    ...
                    weed_images = tf.nn.sigmoid(logits[:, :, :, 3])
                    for i in range(FLAGS.batch_size):
