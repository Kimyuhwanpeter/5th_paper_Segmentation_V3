...
def cal_loss(model, images, labels, objectiness, class_im_plain, ignore_label):
    ...
    with tf.GradientTape() as tape:
        ...
        first_loss = no_obj_loss + (seg_loss + obj_loss)
        #loss = seg_loss
    weights = model.trainable_variables
    grads = tape.gradient(first_loss, model.trainable_variables)
    ...
    with tf.GradientTape() as tape2:
        ...
        no_obj_labels = tf.cast(tf.gather(label_objectiness, no_obj_indices), tf.float32)
        no_obj_loss = -(1. - no_obj_labels) * tf.math.log(1 - tf.nn.sigmoid(no_logit_objectiness) + 1e-7)
        # no_obj_loss = tf.reduce_mean(no_obj_loss + dice_loss(1. - no_obj_labels, 1. - tf.nn.sigmoid(no_logit_objectiness)))
        no_obj_loss = tf.reduce_mean(no_obj_loss)
        ...
        yes_obj_labels = tf.cast(tf.gather(label_objectiness, obj_indices), tf.float32)
        obj_loss = -yes_obj_labels * tf.math.log(tf.nn.sigmoid(yes_logit_objectiness) + 1e-7)
        # obj_loss = tf.reduce_mean(obj_loss + dice_loss(yes_obj_labels, tf.nn.sigmoid(yes_logit_objectiness)))
        obj_loss = tf.reduce_mean(obj_loss)
        ...
        # seg_loss = tf.reduce_mean(-(1. - crop_labels) * ((tf.nn.sigmoid(crop_predict[:, 0:1]))**2) * tf.math.log(1 - tf.nn.sigmoid(crop_predict[:, 0:1]) + 1e-7) + crop_dice_loss) \
        #     + tf.reduce_mean(-weed_labels * ((1-weed_predict[:, 0:1])**2) * tf.math.log(tf.nn.sigmoid(weed_predict[:, 0:1]) + 1e-7) + weed_dice_loss)
        seg_loss = tf.reduce_mean(crop_dice_loss * ((tf.nn.sigmoid(crop_predict[:, 0])**2))*tf.nn.sigmoid_cross_entropy_with_logits(crop_labels, tf.squeeze(crop_predict[:, 0:1], -1))) \
            + tf.reduce_mean(weed_dice_loss * ((1-tf.nn.sigmoid(weed_predict[:, 0]))**2)*tf.nn.sigmoid_cross_entropy_with_logits(weed_labels, tf.squeeze(weed_predict[:, 0:1], -1)))

        loss = seg_loss + (no_obj_loss + obj_loss)
    
    # think slow, take your time, do not hurry!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    grads2 = tape2.gradient(loss, model.trainable_variables + weights)
    func1_ = lambda f, g:[(f[i] + g[i]) for i in range(len(grads))]
    grads3 = func1_(grads, grads2)
    
    optim2.apply_gradients(zip(grads3, model.trainable_variables))
