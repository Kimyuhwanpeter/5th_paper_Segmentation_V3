...
def cal_loss(model, images, labels, objectiness, class_im_plain, gamma_factor, ob_imbal_labels_buf, ignore_label):
    ...
    with tf.GradientTape() as tape2:
        ...
        seg_loss = tf.reduce_mean(class_im_plain[0] * ((tf.nn.sigmoid(crop_predict[:, 0])**2)) *(crop_dice_loss + tf.nn.sigmoid_cross_entropy_with_logits(crop_labels, tf.squeeze(crop_predict[:, 0:1], -1)))) \
            + tf.reduce_mean(class_im_plain[1] * ((1-tf.nn.sigmoid(weed_predict[:, 0]))**2) *(weed_dice_loss + tf.nn.sigmoid_cross_entropy_with_logits(weed_labels, tf.squeeze(weed_predict[:, 0:1], -1))))
