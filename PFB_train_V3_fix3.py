...
def cal_loss(model, images, labels, objectiness, class_im_plain, ignore_label):
    ...
    with tf.GradientTape() as tape:
        ...
        combine_obj_loss = dice_loss(label_objectiness, logit_objectiness) + SigmoidFocalCrossEntropy(tf.expand_dims(label_objectiness, -1), tf.expand_dims(tf.nn.sigmoid(logit_objectiness), -1))
        combine_obj_loss = tf.reduce_mean(combine_obj_loss)

        seg_loss = dice_loss(batch_labels, tf.squeeze(predict[:, 0:1], -1)) \
            + SigmoidFocalCrossEntropy(batch_labels_temp, tf.nn.sigmoid(predict_temp))
        seg_loss = tf.reduce_mean(seg_loss)


        loss = seg_loss + (no_obj_loss + obj_loss + combine_obj_loss) / 3.
