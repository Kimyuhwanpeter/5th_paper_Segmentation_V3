...
FLAGS = easydict.EasyDict({"img_size": 512,
                           ...
                           "val_txt_path": "/yuhwan/yuhwan/Dataset/Segmentation/Crop_weed/CWFID/val.txt",

                           "test_txt_path": "/yuhwan/yuhwan/Dataset/Segmentation/Crop_weed/CWFID/test.txt",
                           
                           "label_path": "/yuhwan/yuhwan/Dataset/Segmentation/Crop_weed/CWFID/aug_train_masks/",
                           
                           "image_path": "/yuhwan/yuhwan/Dataset/Segmentation/Crop_weed/CWFID/aug_train_images/",
                           
                           "pre_checkpoint": False,
                           
                           "pre_checkpoint_path": "/yuhwan/yuhwan/checkpoint/Segmenation/V3_5th_paper/BoniRob/checkpoint/199",
                           ...
                           "sample_images": "/yuhwan/yuhwan/checkpoint/Segmenation/V3_5th_paper/CWFID/sample_images",

                           "save_checkpoint": "/yuhwan/yuhwan/checkpoint/Segmenation/V3_5th_paper/CWFID/checkpoint",

                           "save_print": "/yuhwan/yuhwan/checkpoint/Segmenation/V3_5th_paper/CWFID/train_out.txt",
...
@tf.function
def cal_loss(model, images, labels, objectiness, class_im_plain, gamma_factor, ob_imbal_labels_buf, ignore_label):
    ...
    with tf.GradientTape() as tape2:
        ...
        no_obj_loss = -(1. - no_obj_labels) * tf.math.log(1 - tf.nn.sigmoid(no_logit_objectiness) + 1e-7) * ob_imbal_labels_buf[1] * ((tf.nn.sigmoid(no_logit_objectiness))**2) + dice_loss(1 - no_obj_labels, 1 - tf.nn.sigmoid(no_logit_objectiness))
        ...
        obj_loss = -yes_obj_labels * tf.math.log(tf.nn.sigmoid(yes_logit_objectiness) + 1e-7) * ob_imbal_labels_buf[0] * ((1 - tf.nn.sigmoid(yes_logit_objectiness))**2) + dice_loss(yes_obj_labels, tf.nn.sigmoid(yes_logit_objectiness))
        ...
        seg_loss = tf.reduce_mean(crop_dice_loss + class_im_plain[0] * ((tf.nn.sigmoid(crop_predict[:, 0])**2)) *tf.nn.sigmoid_cross_entropy_with_logits(crop_labels, tf.squeeze(crop_predict[:, 0:1], -1))) \
            + tf.reduce_mean(weed_dice_loss + class_im_plain[1] * ((1-tf.nn.sigmoid(weed_predict[:, 0]))**2) *tf.nn.sigmoid_cross_entropy_with_logits(weed_labels, tf.squeeze(weed_predict[:, 0:1], -1)))
        # seg_loss = dice_loss(batch_labels, tf.nn.sigmoid(tf.squeeze(predict[:, 0:1], -1))) + dice_loss(1 - batch_labels, 1 - tf.nn.sigmoid(tf.squeeze(predict[:, 0:1], -1))) \
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                ...
                loss = cal_loss(model, batch_images, batch_labels, objectiness, temp_imbal, gamma_factor, temp_ob_imbal, 2)
                if count % 10 == 0:
